...
internal class KonanSymbols(
    ...
    val integerConversions = allIntegerClasses.flatMap { fromClass ->
        allIntegerClasses.map { toClass ->
            val name = Name.identifier("to${toClass.descriptor.name.asString().capitalizeFirst()}")
    ...
    val boxCachePredicates = BoxCache.values().associate {
        it to internalFunction("in${it.name.lowercase().capitalizeFirst()}BoxCache")
    }

    val boxCacheGetters = BoxCache.values().associate {
        it to internalFunction("getCached${it.name.lowercase().capitalizeFirst()}Box")
    }
